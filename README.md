...
### 🌡️ 温度偏好设置
- **场景定制**：为通勤、有氧跑、长距离、间歇跑分别设置目标温度
- **个性化推荐**：温度设置即时生效，推荐算法自动适配
- **独立页面**：`/settings/temperature` 单独管理温度配置

### 📤 穿搭分享
- **一键分享**：生成穿搭卡片，包含天气、穿搭详情和二维码
- **跨设备访问**：分享数据持久化存储，链接长期有效
- **社交传播**：支持截图分享至朋友圈/小红书，扫码查看同款穿搭
